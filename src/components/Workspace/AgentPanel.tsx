...
import { Send, Bot, User, Plus, Settings, MessageSquare, Trash2, Brain, Wrench, ChevronDown, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import type {
    Project, AiProviderConfig, Conversation,
    ConversationMessage, ProjectScript,
} from '../../types';
...
export const AgentPanel: React.FC<AgentPanelProps> = ({ project, onNavigate }) => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const [configList, convList] = await Promise.all([
        ...
        <div className="flex flex-col h-full bg-background">
            ...
            <div className="p-3 border-b border-border/40 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => onNavigate?.('workspace')} className="p-1.5 rounded-md text-muted-foreground hover:text-primary hover:bg-muted/80 transition-colors" title="Knowledge Graph">
                        <Brain className="w-4 h-4" />
                    </button>
                    <button onClick={navigateToSettings} className="p-1.5 rounded-md text-muted-foreground hover:text-primary hover:bg-muted/80 transition-colors" title="Provider Settings">
